...
## 📝 Document & Environment Hygiene
- **PRサマリー出力のコードブロック囲み義務（美観と記号維持の保証）**:
  - 【絶対規約】チャット上に GitHub PR 用の説明文を出力する際、平文で出力するとチャットUI上でMarkdownの見出し（`##`）やリスト記号がレンダリングされてしまい、ユーザーがテキストを選択してコピーした際にクリップボードへ記号が欠落して格納される障害が発生する。
  - これを完全に防ぐため、**出力するPRサマリー全体を必ず `markdown` 言語指定のコードブロック（\`\`\`markdown ... \`\`\`）で完全に囲んで出力すること**。これによりユーザーは右上のコピーボタンから生の文字ソースをそのまま取得でき、GitHub側で完璧なレンダリングプレビューが得られる。
- **ワークスペース競合の防止**:
  - プロジェクトルートとサブディレクトリ（`frontend/`等）の双方に `package-lock.json` が重複存在すると、Next.js (Turbopack) がルートディレクトリの推論を誤り起動障害を引き起こす。ルート側で不必要な npm パッケージや lock ファイルの生成を検知した場合は速やかにクリーンアップすること。
