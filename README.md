...
本プロジェクトでは、コードの品質とアーキテクチャのクリーンさを保証するために以下の原則を設けています。

1. **コロケーションの原則**: 特定のページ（例: `/deep-analysis`）でのみ使用されるユーティリティやヘルパー関数（例: `metrics.ts`）は、グローバルな共有ディレクトリではなく、そのページ専用のローカル配下（`app/(dashboard)/deep-analysis/utils/`）に配置してください。
2. **ハイドレーションの防衛**: クライアント専用のデータ（`localStorage` など）はマウント前（SSR時）に同期的に評価せず、`useEffect` 内で非同期に評価し、マウントが完了するまでは同一のスケルトンUIを出力する設計を徹底してください。
3. **型安全性の確保**: `any` 型の使用は禁止されています。動的なプロパティは必ず `unknown` や明確なインターフェースで型定義を行ってください。
