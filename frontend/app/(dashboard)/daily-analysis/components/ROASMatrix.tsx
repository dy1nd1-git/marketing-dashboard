...
export const ROASMatrix: React.FC<ROASMatrixProps> = ({ data }) => {
  ...
  const getCellColor = (roas: number) => {
    ...
    return `rgba(135, 169, 150, ${opacity})`;
  };
  ...
  return (
    <div className="w-full bg-white p-xl rounded-3xl shadow-[0_20px_30px_rgba(135,169,150,0.05)] border border-stone-100">
      <div className="flex justify-between items-center mb-xl">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-outline uppercase tracking-widest font-label">
              Low Efficiency
            </span>
            <div className="flex gap-1">
              {[0.1, 0.3, 0.5, 0.7, 0.9].map((op) => (
                <div
                  ...
                  style={{ backgroundColor: `rgba(135, 169, 150, ${op})` }}
                />
              ))}
            </div>
            <span className="text-[10px] text-outline uppercase tracking-widest font-label">
      ...
      {/* overflow-x-auto のみ — overflow-y は visible のままにするため内側でスクロール管理 */}
          ...
          <div className="flex mb-2">
            ...
            <div className="flex-1 flex justify-between px-1">
              {HOURS.map((h, i) => (
                <span
                  key={h}
                  className="text-[9px] text-outline w-full text-center font-label opacity-60"
                >
                  {i % 3 === 0 ? h : ""}
                </span>
              ))}
                ...
                <div className="flex-1 flex gap-1 h-8">
                  {Array.from({ length: 24 }).map((_, hIdx) => {
                    const cell = data.find(
                      (c) => c.day_of_week === dIdx && c.hour_of_day === hIdx,
                    );
      ...
      {/* Portal tooltip — 親要素のoverflow制約を完全に脱出 */}
      {typeof window !== "undefined" &&
        tooltip &&
        createPortal(
          <div
            className="fixed pointer-events-none z-[9999] px-3 py-1.5 bg-on-surface text-white text-[10px] rounded-lg whitespace-nowrap shadow-xl"
            style={{
              left: tooltip.x,
              top: tooltip.y - 8,
